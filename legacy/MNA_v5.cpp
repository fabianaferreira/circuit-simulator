  ...
  while (fgets(txt,MAX_LINHA,arquivo))
  {
    ...
    else if (tipo == 'I' || tipo == 'V')
    {
      ...
      else if (strcmp(netlist[numeroElementos].tipo_fonte, "PULSE") == 0)
      {
        ...
        elementosVariantes[contadorElementosVariantes] = numeroElementos;
        contadorElementosVariantes++;
        // printf("Tempo de subida antes dos dentro da LerNetlist: %lg\n", netlist[numeroElementos].fonte_pulso.tempo_subida);
        // printf("Tempo de descida antes dos dentro da LerNetlist: %lg\n", netlist[numeroElementos].fonte_pulso.tempo_descida);
      }
      netlist[numeroElementos].a=NumerarNo(na);
      netlist[numeroElementos].b=NumerarNo(nb);
    }
    ...
    for (contador = 0; contador < contadorElementosVariantes; contador++)
    {
      ...
      else if (strcmp(elementoVariante.tipo_fonte, "PULSE") == 0)
      {
        ...
        if (tempo_descida == 0)
         ...
         // printf("Tempo de subida antes dos dentro da MontarEstampasVariantes: %lg\n", tempo_subida);
         // printf("Tempo de descida antes dos dentro da MontarEstampasVariantes: %lg\n", tempo_descida);
        ...
        else
        {
          ...
          else if (tempoReal < tempo_subida + tempo_ligada)
...
void MontarNewtonRaphson (double tempo, double passo_simulacao, unsigned int pontoOperacao) /*muda os elementos nao-lineares de acordo com o resultado anterior*/
{
  ...
  for (contador = 0; contador < contadorElementosNaoLineares; contador++)
  {
    ...
    else if (elementoNaoLinear.nome[0] == 'N')
    {
      ...
      {
        if (elementoNaoLinear.resistorPartes.v2 - elementoNaoLinear.resistorPartes.v1 != 0)
        {
        ...
        {
          if (elementoNaoLinear.resistorPartes.v2 > elementoNaoLinear.resistorPartes.v1)
            g = INFINITO;
          else
            g = -INFINITO;
          z = elementoNaoLinear.resistorPartes.j2 - g*elementoNaoLinear.resistorPartes.v2;
      ...
      {
        if (elementoNaoLinear.resistorPartes.v3 - elementoNaoLinear.resistorPartes.v2 != 0)
        {
        ...
        {
          if (elementoNaoLinear.resistorPartes.v3 > elementoNaoLinear.resistorPartes.v2)
            g = INFINITO;
          else
            g = -INFINITO;
          z = elementoNaoLinear.resistorPartes.j3 - g*elementoNaoLinear.resistorPartes.v3;
      ...
      {
        if (elementoNaoLinear.resistorPartes.v4 - elementoNaoLinear.resistorPartes.v3 != 0)
        {
        ...
        {
          if (elementoNaoLinear.resistorPartes.v4 > elementoNaoLinear.resistorPartes.v3)
            g = INFINITO;
          else
            g = -INFINITO;
          z = elementoNaoLinear.resistorPartes.j4 - g*elementoNaoLinear.resistorPartes.v4;
...
unsigned ResolverNewtonRaphson (double tempo, double passo_simulacao, unsigned int pontoOperacao)
{
  ...
  if (pontoOperacao == 1 && contadorElementosVariantes == 0)
  {
  	for (i=1; i<=numeroVariaveis; i++)
  		newtonRaphsonAnterior[i] = 0.0;
  }
  else
  {
    for (i=1; i<=numeroVariaveis; i++)
...
void ResolverPontoOperacao (double passo_simulacao)
{
  ...
  else /*se tiver elementos nao lineares, resolve com NR*/
  {
    ...
    CalcularMemorias(1, passo_simulacao);
